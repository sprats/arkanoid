...
local background_music = audio.loadStream( "rc/background.mp3" )
local lose_sound = audio.loadSound( "rc/lose.mp3" )
local win_sound = audio.loadSound( "rc/win.mp3" )
local explosion = audio.loadSound( "rc/explosion.mp3" )
...
local base = display.newImage("rc/base.png", display.contentCenterX, display.contentCenterY * 1.7)
physics.addBody( base, "static", { density=1, friction=0.1, bounce=1 } )
base:scale(1, 1)

local ball = display.newImage("rc/ball.png", display.contentCenterX, display.contentCenterY * 1.7)
physics.addBody( ball, "dynamic", { bounce=0.0, radius=25 } )
ball._in_action = false
ball:scale(0.5, 0.5)

local imageSheet = graphics.newImageSheet("rc/bricks.png", options)
